...
public class CustomerController {
    ...
    public String AdminQuestionDetail(@RequestParam("q_group") String question_group, Model model, HttpServletRequest request) {
        ...
        if (request.getRequestURI().contains("loginCheck")) {
            int n = customerService.deleteBtn(question_group);
    ...
    //게시물 삭제(댓글 안 달렸을때만 가능)
    @GetMapping("/loginCheck/questinoDelete")
    public String questinoDelete(String question_id) {
        logger.debug("아이디 : {}" ,question_id);
        customerService.questionDelete(question_id);
        return "redirect:userQuestionList";
    }
